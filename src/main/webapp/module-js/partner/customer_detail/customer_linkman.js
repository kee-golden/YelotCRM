...
require(['jquery', 'yaya','webuploader', 'datatables.net'], function ($, yaya,WebUploader) {
    ...
    var $JCustomerLinkManList = $('#customer_linkMan');

    var table = $JCustomerLinkManList.DataTable({
        "scrollX": true,
        'processing': true,
        'searching': false,
        'lengthChange': false,
        ...
        'columns': [
            {'data': 'major',
                'render':function(data,type, full, meta){
                    if(data == 0){
                        return '<div class="fa fa-star black_star" name='+full.name+'  ></div><input type="hidden" class="majorId" value='+full.id+'>'
                    }else if(data == 1){
                        return '<div class="fa fa-star yellow_star" name='+full.name+' ></div>'
                    }
                }
            },
            {'data': 'name',
    ...
    $JCustomerLinkManList.on('click','.black_star',function () {
            var newName = $(this).attr('name');
            var majorName = $('.yellow_star').attr('name');
            var linkManId = $(this).siblings('.majorId').val();
            $.ajax({
                url:ctx+'/linkMan/major-update',
                data:{linkManId:linkManId,customerId:customerId,newName:newName,majorName:majorName},
    ...
    $JCustomerLinkManList.on('click', '.J_edit', function () {
    ...
    $('#customer_linkManAdd').click(function () {
        $.ajax({
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (data.code) {
                                        yaya.layer.msg("保存成功");
                                        table.draw();
                                        var linksize=table.page.info().recordsTotal+1;
                                        $("#linkMan").text("联系人（"+linksize+"）");
                                    }
                                },
                                error: function () {
                                    yaya.layer.msg("系统异常");
